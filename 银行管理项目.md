...
- 类型与令牌之间必须是以空格为间隔的
以下是gin获取的方式，首先通过上下文获取这个标头的信息
```go
authorizationHeader := ctx.GetHeader(authorizationHeaderKey)
```
随后需要根据空格来区分实际的类型与令牌实体
```go
//按空格来解析令牌
fields := strings.Fields(authorizationHeader)
```
之后获取到一个二个元素的数组 类型需要换成小写，其余的令牌实体直接取即可
...
###### 这个生成器只能生成接口，具体是业务逻辑必须由操作者去实现、重写
```go
...
##### sh脚本文件的行尾序列 
- 必须是LF的形式 才能在容器里面运行 否则就会报：exec:/app/wait-for.sh No such file or directory


##### 在使用sqlc的时候，有时候更新全部信息更加的简单，但是要是用户只想更新其中的某一个呢？
###### 第一种
- 使用CASE的语句来制定分支条件
- 并且需要用@来指定是什么结构体名字，并且指定类型是什么,也就是当什么条件之下，满足什么就是什么
```sql
UPDATE users
SET
...
```
###### 第二种
利用sqlc的语法,直接如果为空，就不对其进行修改
sqlc.narg((clomn),clomn)
```sql
UPDATE users
SET
...
##### 那在GRPC里面，怎么添加限制的字段，也就是当其余的元素不传输的时候
- 在proto文件中，在类型前面加一个options的字段，就可以，在生成的代码的结构体里面，就有一个*string类型
```go
...
```
表示是可以空缺的类型(主要是跟上面sqlc生成的代码结构进行联动)
```go
...
```
这个sql.Null...类型就是一个结构体，用于判断是否为空的，若不填写任何信息，默认值是空


##### 那在GRPC里面，添加日志输出文件（一般的zap库等等日志库，都会不会打印输出信息，因为被直接调用了函数）---> 所以需要使用GRPC中zerolog的包
需要一个grpc options
...
##### 但是网关的拦截http请求，在HTTP和GRPC的进程之间进行转换；会直接调用grpc的函数，并不会经过什么拦截器,因此需要使用中间件来处理这个日志
- 使用到handler,在处理的函数，添加一个处理中间件handler，接受一个handler参数，返回一个handler对象
```go
...
```
可以从http的request结构体里面，获取很多信息
在一个结构体里面，嵌入另一个结构，那么，另一个结构的所有接口方法都可以直接使用,这个是接口，所以需要对其进行接口的方法。来对我需要的结构体进行赋值。并且此时是可以以我自己定义的类，放到需要我属性的类的参数
```go
...
##### 使用Redis来处理请求，将所有的请求任务都放到redis里面,后端按需在redis的任务序列中取任务
- 使用工具 Asynq 是一个分布式的任务队列库，异步的排队任务。可以精确設置任务的属性（截止日期、运行时间等等）
